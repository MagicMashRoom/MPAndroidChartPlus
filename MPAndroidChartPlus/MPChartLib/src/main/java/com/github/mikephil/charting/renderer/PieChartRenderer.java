...
public class PieChartRenderer extends DataRenderer {
    ...
    public PieChartRenderer(PieChart chart, ChartAnimator animator,
        ...
        mTransparentCirclePaint.setAlpha(209);//白色透明度82%
    ...
    protected void drawHole(Canvas c) {
        ...
        if (mChart.isDrawHoleEnabled() && mBitmapCanvas != null) {
            ...
            if (Color.alpha(mTransparentCirclePaint.getColor()) > 0 &&
                    mChart.getOuterDecorateRadius() > 0f) {
                int alpha = mTransparentCirclePaint.getAlpha();
                mTransparentCirclePaint.setAlpha((int) ((float) alpha * mAnimator.getPhaseX() * mAnimator.getPhaseY()));

                float outerDecorateSize = radius * (mChart.getOuterDecorateRadius() / 100);
                mTransparentCirclePaint.setStrokeWidth(outerDecorateSize);
                mHoleCirclePath.reset();
                mHoleCirclePath.addCircle(center.x, center.y, radius, Path.Direction.CW);
                mHoleCirclePath.addCircle(center.x, center.y, radius - outerDecorateSize, Path.Direction.CCW);
//                mHoleRect.set(center.x - holeRadiusOuter - outerDecorateSize / 2, center.y - holeRadiusOuter - outerDecorateSize / 2,
//                        center.x + holeRadiusOuter + outerDecorateSize / 2, center.y + holeRadiusOuter + outerDecorateSize / 2);
//                mHoleCirclePath.addArc(mHoleRect , -180 , 180);
