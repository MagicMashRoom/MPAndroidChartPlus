...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            HorizontalBarChart barChart = (HorizontalBarChart) mChart;
            if(barChart.isDrawItemBitmapArrow() && index == 1) {
                Bitmap bitmap = barChart.getItemArrow();
                if(bitmap != null) {
                    c.drawBitmap(bitmap ,
                            mViewPortHandler.contentRight() - Utils.convertDpToPixel(15f) ,
                            buffer.buffer[j + 1] / 2 + buffer.buffer[j + 3] / 2 + 6,
                            mRenderPaint);
                }
            }
        }
